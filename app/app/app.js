...
myApp.controller('MainController', ['$scope', '$transitions','$http', '$anchorScroll', '$location', '$stateParams', '$timeout', '$state', '$rootScope', '$window', function ($scope, $transitions, $http, $anchorScroll, $location, $stateParams, $timeout, $state, $rootScope, $window)  {
  ...
  $scope.submitForm = function(formType){
    ...
    $scope.loading = true;
    $http.post('/sendmail', {
      from: '"ITNNorthJersey Web User" <donotreply@itnamerica.com>',
      to: 'itnamerica2018@gmail.com',
      subject: "ITNNorthJersey Contact Form Submitted",
      text: $scope.formData,
  ...
  $scope.submitFormWithPDF = function(formType){
    ...
    if (formType === 'volunteer') {
        ...
        $scope.formSubject = 'ITNNorthJersey - New volunteer application received';
        ...
        $scope.showPdf = true;
        $scope.formSubject = 'ITNNorthJersey - New membership application received';
        $scope.generateMultiPagePDF();
    } else if (formType === 'nonrider') {
        $scope.formSubject = 'ITNNorthJersey - Non-Rider application Form submitted';
        $scope.generatePDF();
    } 
  }
